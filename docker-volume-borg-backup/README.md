...
These instructions assume you are testing things up and running everything from a single machine running on ubuntu. If not you will likely need to adapt them.
...
ssh-keygen -f ~/.ssh/id_ed25519_borg_backup -t ed25519 -N ''
cat ~/.ssh/id_ed25519_borg_backup > secret_backup_server_user_private_key.txt
chmod 400 secret_backup_server_user_private_key.txt
cat ~/.ssh/id_ed25519_borg_backup.pub >> ~/.ssh/authorized_keys
chmod g-w ~
chmod 700 ~/.ssh
...
```
backup_cron_1 | KEEPALIVE CHECK [2026-01-10 01:30:01]
```
...
Check the repo directly for more information at [https://github.com/Ovski4/docker-borgbackup-cron](https://github.com/Ovski4/docker-borgbackup-cron)